...
const fetchEmployees = async () => {
  try {
    const response = await fetch(
      "http://127.0.0.1:8080/Payments/listPaymentRecords",
      {
        method: "GET",
...
function setDataPage() {
  if (Payments.value.length > 0) {
    PaginationData.value.NumberPage = Math.ceil(
      Payments.value.length / PaginationData.value.elements
    );
  } else {
...
function getSectionEmployee() {
  PaginationData.value.sectionPayments = [...Payments.value].slice(
    PaginationData.value.init,
        ...
        >
          ...
          <div class="flex items-center justify-between p-3">
            <p class="block text-sm text-slate-500">
              Pagina {{ PaginationData.Page }} de
              {{ PaginationData.NumberPage }}
            </p>
